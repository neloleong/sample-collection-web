...
function getRegionBarColor(quantity: number, quota: number) {
  if (quota > 0 && quantity >= quota) {
    return "bg-red-600";
  }
  return "bg-green-600";
}

function getRegionTextColor(quantity: number, quota: number) {
  if (quota > 0 && quantity >= quota) {
    return "text-red-700";
  }
...
export default function ReportManagementPage() {
  ...
  const totalBarColor = useMemo(() => {
    if (totalSuggestedQuota > 0 && aggregated.totalScore >= totalSuggestedQuota) {
      return "bg-red-600";
    }
    return "bg-green-600";
  }, [aggregated.totalScore, totalSuggestedQuota]);
            ...
            <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
              ...
              <div className="mt-6">
                <div className="mb-2 flex items-center justify-between text-sm">
                  ...
                  <span
                    className={`font-semibold ${
                      totalSuggestedQuota > 0 && aggregated.totalScore >= totalSuggestedQuota
                        ? "text-red-700"
                        : "text-green-700"
                    }`}
                  >
                ...
                <div className="h-3 w-full rounded-full bg-slate-200">
                  <div
                    className={`h-3 rounded-full transition-all ${totalBarColor}`}
            ...
            <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
              ...
              <div className="mt-6 space-y-5">
                {regionStats.map((region) => {
                  const barColor = getRegionBarColor(region.quantity, region.quota);
                  const textColor = getRegionTextColor(region.quantity, region.quota);
